...
namespace eval sqlk {
  ...
  proc kinit {filepath args} {
    ...
    namespace eval $dbid {
      ...
      proc dodecbin  {data}                   {::sqlk::_dodecbin}
      ...
      namespace export varlist varadd vardel keys kget kset kdel kexist kmove attrset attrget attrdel serialize closefile varcmd parse tree backup restore
  ...
  proc _kget {} {
    uplevel 1 {
      ...
      if {$encrypt} {
        return [dodecbin [$db onecolumn {select val from key_data where id = $p}]]
      } else {
  ...
  proc _attrset {} {
    uplevel 1 {
      ...
      if {$encrypt} {
        array set a [dodecbin [$db onecolumn {select attr from key_data where id = $p}]]
      } else {
  ...
  proc __serialize {ns name oformat oindent okey {level 1}} {
    if {$oformat == "TCL"} {
      ...
      # TODO: XML serialization pending
    }
  }
  ...
  ##########
  #
  # AES encryption: deterministic IV for searchable names,
  # random IV for values/attributes.
  #
  proc _doenc {} {
    uplevel 1 {
      variable kenckey
      if {$kenckey == ""} {
        ...
        # Deterministic IV (all zeros) for names — must be searchable
        set iv [string repeat \x00 16]
        set key16 [string range [string repeat \x00 16] 0 15]
        set key16 [string replace $key16 0 [expr {[string length $kenckey]-1}] $kenckey]
        return [binary scan [::aes::aes -dir encrypt -key $key16 -iv $iv -- $data] H* hex ; set hex]
      }
    }
  }
  ...
  proc _dodec {} {
    uplevel 1 {
      ...
      if {$kenckey == ""} {
        ...
        set key16 [string range [string repeat \x00 16] 0 15]
        set key16 [string replace $key16 0 [expr {[string length $kenckey]-1}] $kenckey]
        return [::aes::aes -dir decrypt -key $key16 -iv $iv -- [binary format H* $data]]
      }
    }
  }
  ...
  ##########
  #
  # Random IV for values/attributes — stored as IV+ciphertext in binary.
  #
  proc _doencbin {} {
    uplevel 1 {
      variable kenckey
      if {$kenckey == ""} {
        ...
        return $iv[::aes::aes -dir encrypt -key $key16 -iv $iv -- $data]
      }
    }
  }
  ...
  proc _dodecbin {} {
    uplevel 1 {
      ...
      if {$kenckey == ""} {
        ...
        set key16 [string range [string repeat \x00 16] 0 15]
        set key16 [string replace $key16 0 [expr {[string length $kenckey]-1}] $kenckey]
        set iv    [string range $data 0 15]
        set ctext [string range $data 16 end]
        return [::aes::aes -dir decrypt -key $key16 -iv $iv -- $ctext]
      }
    }
  }
  ...
  proc _keyid {} {
    uplevel 1 {
      variable db
      $db onecolumn {select id from key_data where name_id = $name_id and path = $key}
    }
  }
  ...
  #
  proc validname {name} {
    return [expr ![regexp -- {^[[:alnum:]][[:alnum:]_-]{1,40}$} $name]]
  }
  ...
  #
  proc validkey {name} {
    return [expr ![regexp -- {^[[:alnum:]][[:alnum:]_.-]{0,}$} $name]]
  }
  ...
  proc initkdb {db} {
    set tables [$db eval {
        ...
        WHERE type = 'table'
          AND (name = 'key_data' OR name = 'key_name')
        ORDER BY name
    }]
	...
	CREATE TABLE "key_data" (
        ...
        CREATE INDEX key_data_key2 on key_data (name_id,path);
	        ...
	        after_id =
	         (select a.last_id from key_name as a where id = NEW.name_id),
                key_order =
                 NEW.id,
                path =
                 ifnull((select p.name || '.' from key_data as p where p.id = NEW.parent),'') || NEW.name,
                ctime = strftime('%s'),
                utime = strftime('%s')
	      where
	        id = NEW.id;
